...
per_generator = defaultdict(lambda: {
    "image_correct": 0,
    "video_correct": 0,
    "fusion_correct": 0,
    "count": 0
...
for i, row in enumerate(tqdm(rows)):
    ...
    if i < 5:
        print(f"\nSample {i}")
        print("GT:", label)
        print("Image:", img_pred)
        print("Video:", vid_pred)
        print("Fusion:", fus_pred)
    ...
    if label == 0:
        ...
        per_generator[gen]["count"] += 1
        per_generator[gen]["image_correct"] += int(img_pred == label)
        per_generator[gen]["video_correct"] += int(vid_pred == label)
        per_generator[gen]["fusion_correct"] += int(fus_pred == label)
...
for gen in sorted(per_generator.keys()):
    data = per_generator[gen]
    count = data["count"]

    if count == 0:
        continue

    image_acc = data["image_correct"] / count
    video_acc = data["video_correct"] / count
    fusion_acc = data["fusion_correct"] / count

    print(f"\nGenerator: {gen}")
    print(f"  Image  Fake Acc: {round(image_acc, 4)}")
    print(f"  Video  Fake Acc: {round(video_acc, 4)}")
    print(f"  Fusion Fake Acc: {round(fusion_acc, 4)}")