...
df_cnn = pd.read_csv("/workspace/benchmarks_ai_research/routing/sd14_bandit_dataset.csv")
df_qwen = pd.read_csv("/workspace/benchmarks_ai_research/routing/sd14_qwen_preds.csv")

df_clip = pd.read_csv("/workspace/benchmarks_ai_research/routing/sd14_clip_preds.csv")
df = df_cnn.merge(df_qwen[["path", "y_qwen"]], on="path")
df = df.merge(df_clip[["path", "y_clip", "clip_proba"]], on="path")

val_paths = np.load("/workspace/benchmarks_ai_research/routing/sd14_bandit_val_paths.npy", allow_pickle=True)
...
clip_only_acc = np.mean(df["y_clip"] == df["y_true"])

print(f"CLIP-only Accuracy: {clip_only_acc:.4f}")
