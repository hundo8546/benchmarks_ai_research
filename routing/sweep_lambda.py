...
df_clip = pd.read_csv("clip_preds.csv")
...
def evaluate_lambda(LAMBDA):
    ...
    X = df_local[["logit", "confidence", "margin1", "entropy1", "flip","disagree"]].values
    ...
    for _, row in df_local.iterrows():
        context = np.array([[row["logit"], row["confidence"], row["margin1"], row["entropy1"], row["flip"],row["disagree"]]])
